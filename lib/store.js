...
function defaultData() {
  ...
  return {
    ...
    cartItems: [],
    faqs: [
      { id: 1, question: '배송은 얼마나 걸리나요?', answer: '결제 완료 후 평균 2~3일 이내 출고됩니다.', sort_order: 1, user_id: null, created_at: now, answered_at: now },
      { id: 2, question: '교환/환불은 어떻게 하나요?', answer: '마이페이지 > 주문내역에서 신청 가능하며, 상품 수령 후 7일 이내 가능합니다.', sort_order: 2, user_id: null, created_at: now, answered_at: now },
      { id: 3, question: '사이즈는 어떻게 확인하나요?', answer: '각 상품 상세페이지 하단 사이즈 가이드를 참고해주세요.', sort_order: 3, user_id: null, created_at: now, answered_at: now },
    ],
    _seq: { users: 0, products: 5, reviews: 0, orders: 0, cartItems: 0, faqs: 3 },
  };
}
...
if (!db) {
  ...
  // 관리자 계정: .env의 ADMIN_USERNAME / ADMIN_PASSWORD 값으로 매 부팅마다 강제 동기화
  // (db.json에 예전 비밀번호가 남아있어도 재시작하면 항상 .env 값으로 로그인 가능해짐)
...
// 하위 호환 처리: 이전 버전 db.json에는 없던 필드/시퀀스를 보정
if (db._seq.faqs === undefined) {
  db._seq.faqs = db.faqs.reduce((max, f) => Math.max(max, f.id), 0);
}
db.reviews.forEach((r) => {
  if (r.admin_reply === undefined) r.admin_reply = null;
  if (r.replied_at === undefined) r.replied_at = null;
});
db.faqs.forEach((f) => {
  if (f.user_id === undefined) f.user_id = null;
  if (f.created_at === undefined) f.created_at = new Date().toISOString();
  if (f.answered_at === undefined) f.answered_at = f.answer ? new Date().toISOString() : null;
});
...
export function replyToReview(reviewId, content) {
  const review = db.reviews.find((r) => r.id === Number(reviewId));
...
// ---------------- FAQ ----------------
function withAsker(faq) {
  const asker = faq.user_id ? findUserById(faq.user_id) : null;
  return { ...faq, asker_name: asker?.name || null };
}

// 공개 목록: 기본적으로 답변이 등록된 질문만, sort_order -> 최신순
export function listFaqs({ onlyAnswered = true } = {}) {
  let items = db.faqs;
  if (onlyAnswered) items = items.filter((f) => !!f.answer);
  return items
    .slice()
...
// 관리자 전용: 미답변 질문을 위로 올려서 전체 조회
export function listFaqsForAdmin() {
  return db.faqs
    .slice()
    .sort((a, b) => {
      if (!!a.answer !== !!b.answer) return a.answer ? 1 : -1; // 미답변 먼저
      return new Date(b.created_at) - new Date(a.created_at);
    })
    .map(withAsker);
}

// 사용자가 질문 등록 (답변 대기 상태로 생성)
export function askFaqQuestion({ user_id, question }) {
  const faq = {
    id: nextId('faqs'),
    question,
    answer: null,
    sort_order: db.faqs.length + 1,
    user_id,
    created_at: new Date().toISOString(),
    answered_at: null,
  };
...
// 관리자가 질문에 답변 등록/수정
export function answerFaq(id, answer) {
  const faq = db.faqs.find((f) => f.id === Number(id));
  if (!faq) return null;
  faq.answer = answer;
  faq.answered_at = new Date().toISOString();
  persist();
  return faq;
}

// 관리자가 공식 FAQ 직접 등록 (질문+답변 동시 입력)
...
export function deleteFaq(id) {
  const idx = db.faqs.findIndex((f) => f.id === Number(id));
  if (idx === -1) return false;
